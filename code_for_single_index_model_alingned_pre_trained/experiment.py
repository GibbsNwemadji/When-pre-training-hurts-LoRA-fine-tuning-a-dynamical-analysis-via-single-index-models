...
# ============================================================
# Single-index teacher–student with pretrained init w0(μ)=μ ω★
# ============================================================
#
# This file runs a SINGLE-INDEX regression model (K = 1 by default).
#
# Teacher model:
#   - sample x ~ N(0, I_N)
#   - teacher direction ω★ ∈ R^N (called T in code, normalized)
#   - label: y = σ( x · ω★ )
#
# Student model:
#   - student direction w ∈ R^N (called S in code)
#   - prediction: y_hat = σ( x · w )
#
# Pretrained initialization (this variant):
#   - student starts from a scaled teacher direction:
#         w0(μ) = μ ω★
#   - μ ∈ (0,1] is the "pretraining strength"
#
# LoRA adaptation (optional):
#   - we add a rank-r correction U V on top of the pretrained init:
#         w = w0(μ) + U V
#
# In the CLI / bash scripts, the list `d_list` is used to sweep μ.
# (We keep the name d_list only for backward compatibility.)
#
# ============================================================
...
def save_overlap_plot(overlap_history, seed, mu, result_dir):
    """Save overlap trajectory between V and teacher direction."""
    plt.figure()
...
# Single seed run
# ------------------------------------------------------------
def single_seed_run(seed, args, T_cpu, X_test_cpu, y_test_cpu, mu, result_dir):
    """
    ...
    # =========================================================
    if mu == "V_only":
        # Train the student direction directly: S = V
        ...
        # Pretrained initialization: w0(μ) = μ ω★
        w0 = float(mu) * T

        # LoRA adaptation: S = w0 + U V  (rank r)
    ...
    max_epochs = getattr(args, "max_epochs", 1000)
    ...
    while current_epoch < total_epochs:
        ...
        with torch.no_grad():
            ...
            # overlap(V, teacher) is the main order parameter in the search phase
            ...
            # In single-index, this is essentially ⟨S, T⟩
...
# ------------------------------------------------------------
# Main experiment (parallel over seeds, sweep μ)
# ------------------------------------------------------------
def run_experiment(args):
    ...
    # μ sweep list (still called d_list in args)
    mu_list = []
    ...
    for mu in mu_list:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(single_seed_run, seed, args, T, X_test, y_test, mu, result_dir)
...
# ------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Single-index teacher–student with pretrained init w0(μ)=μ ω★ (parallelized)")
    parser.add_argument(
        "--activation",
        type=str,
        default="linear",
